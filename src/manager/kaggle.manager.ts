...
import { BABY_NAMES_PAGE_URL, KAGGER_BASE_URL, NUMBER_REGEX, YEAR_OF_BIRTH_REGEX } from '../common/constant'
...
export class KaggleBabyName {
    ...
    constructor({ Name, Sex, YearOfBirth, Number }: any) {
        this.Name = Name
        this.Sex = Sex
...
export class KaggleManager {
    ...
    /**
     * Goes to the KAGGER_BASE_URL and logs in using the provided credentials.
     * @param username
     * @param password 
     */
    async kaggleLogin(username: string, password: string) {
        ...
        try {
            await page.goto(KAGGER_BASE_URL)
            ...
            throw new KaggleError(err.message || 'Unknown error')
        } finally {
            await page.close()
        }
    }
